...
export default class MusiciansAdmin extends Component {
  ...
  handleAddMusician = async (id, event) => {
    ...
    try {
      ...
      this.setState({
        musicians: [...this.state.musicians, this.state.newmusician],
      });
      this.setState({
        newmusician: { firstName: "", lastName: "", genre: "" },
      });
    } catch (err) {
      console.log(err);
    }
  ...
  handleUpdateMusician = (id, name) => {
    ...
    const musicianToUpdate = [...this.state.musicians].find(
      (musician) => musician.id === id
    );
    const updatedMusicians = [...this.state.musicians].filter(
      (musician) => musician.id !== id
    );
  ...
  handleDeleteMusician = async (id, event) => {
    ...
    try {
      ...
      const updatedMusicians = await [...this.state.musicians].filter(
        (musician) => musician.firstName.toLowerCase() !== id
      );
      this.setState({ musicians: updatedMusicians });
    } catch (err) {
      console.log(err);
  ...
  onAddMusicianFirstNameChange = (event) =>
    this.setState({
      newmusician: { ...this.state.newmusician, firstName: event.target.value },
    });
  onAddMusicianLastNameChange = (event) =>
    this.setState({
      newmusician: { ...this.state.newmusician, lastName: event.target.value },
    });
  onAddMusicianGenreChange = (event) =>
    this.setState({
      newmusician: { ...this.state.newmusician, genre: event.target.value },
    });
          ...
          <div className="container">
            ...
            <div className="columns">
              <div className="column is-one-third">
                <form
                  onSubmit={(event) =>
                    this.handleAddMusician(
                      this.state.newmusician.firstName.toLowerCase(),
                      event
                    )
                  }
                >
