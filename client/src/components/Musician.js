...
export default class MusicianAdmin extends Component {
  ...
  handleEditSave = (event) => {
    ...
    this.props.handleUpdateMusician(
      this.props.id,
      this.state.updatedmusicianname
    );
  };

  onAddMusicianNameChange = (event) =>
    this.setState({ updatedmusicianname: event.target.value });
      ...
      <div className="box musician-card notification is-success">
        {this.props.isAdmin && (
          <Fragment>
            <button
              onClick={(event) =>
                this.props.handleDeleteMusician(this.props.id, event)
              }
              className="delete"
            ></button>
          </Fragment>
